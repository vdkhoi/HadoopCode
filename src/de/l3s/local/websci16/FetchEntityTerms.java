...
public class FetchEntityTerms {
	...
	public static void main(String agrs[]) {
		try {
			...
			File o = new File(
					"Z:/Alexandria/Results/Result_follow_up_WebSci16/entity_terms.txt");
			...
			while (rs.next()) {
				...
				while (tokens.incrementToken()) {
					...
					if (term.length() > 2 && !term.matches("^[-+]?\\d+([\\.\\,]\\d+)*$") && !stopwE.isStopword(term) && !stopwG.isStopword(term)) {
						wr.write(term + "\r\n");
					}
