...
public class MergeIndexAnchorFromDataset extends Configured implements Tool {
		...
		public void map(LongWritable key, Text value, Context output)
			...
			try {
				if (value != null) {
					...
					//if (! line[0].equals("de,arcor,home)/foederalismus/ges/wyss-samen-und-pflanzen.html")) return;
		...
		protected void reduce(Text key, Iterable<Text> values, Context output)
			...
			try {
				...
				while (iter.hasNext()) {
					...
					inlink ++;
					len = anchor_list.get(currentAnchor);
					if ( len < MAX_LENGTH) {
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "MergeIndexAnchorFromDataset");
		...
		job.setJarByClass(MergeIndexAnchorFromDataset.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new MergeIndexAnchorFromDataset(), args);
		System.exit(res);
	}
}