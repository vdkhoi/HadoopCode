...
public class BuildTermFrequencyRevisionData extends Configured implements Tool {
			...
			try {
				...
				if (line.length == 4) {
					Tokenizer tokenizer = new StandardTokenizer(
							new StringReader(StringEscapeUtils.unescapeHtml(
									line[3].trim()).toLowerCase()));
					CharTermAttribute charTermAttrib = tokenizer
							.getAttribute(CharTermAttribute.class);
					tokenizer.reset();
					while (tokenizer.incrementToken()) {
						textKey = new Text(line[0].trim() + "\t" + line[1]
								+ "\t" + line[2] + "\t"
								+ charTermAttrib.toString());
	...
	private static class Combiner extends
		...
		protected void reduce(Text key, Iterable<LongWritable> values,
			...
			output.write(new Text(line[2] + "\t" + line[3]), new LongWritable(
					sum));
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(BuildTermFrequencyRevisionData.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new BuildTermFrequencyRevisionData(), args);
		System.exit(res);
	}
}