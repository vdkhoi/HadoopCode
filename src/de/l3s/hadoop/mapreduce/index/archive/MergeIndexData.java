...
public class MergeIndexData extends Configured implements Tool {
	...
	private static class Reduce extends Reducer<Text, Text, Text, Text> {
		...
		protected void reduce(Text key, Iterable<Text> values,	Context output)
			...
			while (iter.hasNext()) {
				...
				if (anchors.indexOf(currentAnchor) < 0)
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "MergeIndexData");
		...
		job.setJarByClass(MergeIndexData.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new MergeIndexData(), args);
		System.exit(res);
	}
}