...
public class BuildTermFrequencyNoRevisionData extends Configured implements Tool {
	...
	private static class Map extends
		...
		public void map(LongWritable key, Text value, Context output)
			...
			try {
				...
				if (line.length == 3) {
					Tokenizer tokenizer = new StandardTokenizer(new StringReader(StringEscapeUtils.unescapeHtml(line[2].trim()).toLowerCase()));
					CharTermAttribute charTermAttrib = tokenizer
							.getAttribute(CharTermAttribute.class);
					tokenizer.reset();
					while (tokenizer.incrementToken()) {
						textKey = new Text(line[1].trim() + "\t" + charTermAttrib.toString());
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "BuildTermFrequencyRevisionData");
		...
		job.setJarByClass(BuildTermFrequencyNoRevisionData.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new BuildTermFrequencyNoRevisionData(), args);
		System.exit(res);
	}
}