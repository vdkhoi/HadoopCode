...
public class MergeIndexExpandedData extends Configured implements Tool {
	...
	private static class Map extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, Context output)
			...
			try {
				...
				if (line.length == 8) {
					textKey = new Text(line[6].trim());
					textValue = new Text(StringEscapeUtils.unescapeHtml(line[7]
							.trim()));
					output.write(textKey, textValue);
				}
				if (line.length == 7) {
					textKey = new Text(line[5].trim());
					textValue = new Text(StringEscapeUtils.unescapeHtml(line[6]
							.trim()));
					output.write(textKey, textValue);
				}
				...
				throw new InterruptedException(value.toString() + "\r\n" + e1);
			}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "MergeIndexExtendedData");
		...
		job.setJarByClass(MergeIndexExpandedData.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new MergeIndexExpandedData(), args);
		System.exit(res);
	}
}