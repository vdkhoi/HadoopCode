...
public class MergeTimeStampFromDataset extends Configured implements Tool {
		...
		public void map(LongWritable key, Text value, Context output)
			...
			try {
				if (value != null) {
					...
					if (line.length == 4 && line[3].length() > 0 && line[2].length() > 0) {
						textKey = new Text(line[2].trim());
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(MergeTimeStampFromDataset.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new MergeTimeStampFromDataset(), args);
		System.exit(res);
	}
}